...
namespace ZBobb
{
	...
	public class AlphaBlendTextBox : TextBoxMaskBox
	{
...
//----
			...
			if (this.Focused && (this.MaskBoxSelectionLength == 0))
			{
		...
		private Point findCaret() 
		{
			...
			int i_char_loc = this.MaskBoxSelectionStart;
			...
			else if (i_char_loc >= this.MaskBoxText.Length)
			{
				...
				Graphics g = this.CreateGraphics();
				String t1 = this.MaskBoxText.Substring(this.MaskBoxText.Length-1,1) + "X";
				...
				if (i_char_loc == this.MaskBoxText.Length)
				{
					String slast = this.MaskBoxText.Substring(MaskBoxText.Length-1,1);
					if (slast == "\n")
					{
						pointCaret.X = 1;
		...
		{
			get { return OwnerEdit.Properties.BackAlpha; }
		}

		#endregion

        public new AlphaBlendTextEdit OwnerEdit {
            get { return (AlphaBlendTextEdit)base.OwnerEdit; }
        }
